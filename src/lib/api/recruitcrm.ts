...
export class RecruitCRMService {
  ...
  async syncJobs() {
    ...
    try {
      ...
      while (true) {
        const response = await fetch(`${API_BASE}/jobs?page=${page}&per_page=100`, {
          headers: {
            'Authorization': `Bearer ${this.apiKey}`,
            'Content-Type': 'application/json',
            'Accept': 'application/json',
        ...
        for (const job of data) {
          try {
            ...
            if (this.filters) {
              if (this.filters.status?.length && !this.filters.status.includes(job.status)) {
                continue;
              }
              if (this.filters.jobTypes?.length && !this.filters.jobTypes.includes(job.job_type.name)) {
                continue;
              }
              if (this.filters.locations?.length && !job.locations.some(loc => 
...
const MOCK_JOBS = [
  {
    ...
    description: 'Leading fintech company seeking an experienced Product Manager to drive innovation in our payment solutions...',
    status: 'active',
  ...
  },
  {
    ...
    description: 'Join our Web3 team to build next-generation decentralized applications...',
    status: 'active',
    job_type: { id: 1, name: 'Full Time' },
    locations: [
