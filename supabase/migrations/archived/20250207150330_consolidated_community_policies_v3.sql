...
---
id: "20250207150330"
title: "Consolidated Community Policies V3"
description: >
  ...
  permissive policies for all other operations. Policy names match test expectations exactly.
  Handles the case where a user exists in auth.users but not in profiles.
affected_tables:
  - "public.communities"
...
CREATE POLICY "Community owners can create communities"
    ...
    WITH CHECK (
        EXISTS (
            ...
            AND (users.raw_user_meta_data->>'role')::text = 'owner'
        )
    );
